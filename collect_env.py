...
# Run it with `python collect_env.py` or `python -m tvm.collect_env`
...
SystemEnv = namedtuple(
    ...
    [
        ...
        "cpu_info",
        "tvm_version",  # tvm specific field
        "tvm_file",  # tvm specific field
        "tvm_libinfo",  # tvm specific field
        "env_vars",
    ],
)

DEFAULT_CONDA_PATTERNS = {
    # base requirements for tvm
    "build",
    "cloudpickle",
    "cython",
    "decorator",
...
def get_env_info():
    ...
    tvm_file = tvm.__file__
    tvm_libinfo = tvm.support.libinfo()
    tvm_libinfo_str = ""
    width = max(len(k) for k in tvm_libinfo)
    for key, value in tvm_libinfo.items():
        tvm_libinfo_str += f"{key:<{width}} : {value}\n"
    ...
    return SystemEnv(
        ...
        tvm_version=tvm_version,  # tvm specific field
        tvm_file=tvm_file,  # tvm specific field
        tvm_libinfo=tvm_libinfo_str,  # tvm specific field
